...
if($page>=$totalPage){ 
  ...
  $sql = "select id,title,description from info_article where title like '%$key%' order by dateline desc limit {$offset},{$pageSize}";
  $rows = fetchAll($sql);
?>
  ...
  <div class="row row-offcanvas row-offcanvas-right">
    <div class="col-xs-12 col-sm-9" style="background:#fff;">
      ...
      <?php if($rows){ foreach ($rows as $row):?>
        ...
        <hr>
        <?php endforeach;} else{ echo "<h2 style='margin-top:30px;'>没有搜索结果！</h2>";}?>
      <!--page-->
